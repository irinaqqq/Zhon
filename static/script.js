...
function getFirstCoord(e) {
	var 	carPos = firstName.selectionEnd,
		div = document.createElement('div'),
		span = document.createElement('span'),
		copyStyle = getComputedStyle(firstName),
		emailCoords = {}, caretCoords = {}, centerCoords = {}
	;
	[].forEach.call(copyStyle, function(prop){
		div.style[prop] = copyStyle[prop];
	...
	div.style.position = 'absolute';
	document.body.appendChild(div);
	div.textContent = firstName.value.substr(0, carPos);
	span.textContent = firstName.value.substr(carPos) || '.';
	div.appendChild(span);
	
	emailCoords = getPosition(firstName);							
	caretCoords = getPosition(span);							
	centerCoords = getPosition(mySVG);							
	svgCoords = getPosition(mySVG);
	screenCenter = centerCoords.x + (mySVG.offsetWidth / 2);		
...
function getLastCoord(e) {
	var 	carPos = lastName.selectionEnd,
		div = document.createElement('div'),
		span = document.createElement('span'),
		copyStyle = getComputedStyle(lastName),
		emailCoords = {}, caretCoords = {}, centerCoords = {}
	;
	[].forEach.call(copyStyle, function(prop){
		div.style[prop] = copyStyle[prop];
	...
	div.style.position = 'absolute';
	document.body.appendChild(div);
	div.textContent = lastName.value.substr(0, carPos);
	span.textContent = lastName.value.substr(carPos) || '.';
	div.appendChild(span);
...
function getUserCoord(e) {
	...
	emailCoords = getPosition(username);							
	caretCoords = getPosition(span);							
	centerCoords = getPosition(mySVG);							
	svgCoords = getPosition(mySVG);
	screenCenter = centerCoords.x + (mySVG.offsetWidth / 2);		
...
function onLastFocus(e) {
	e.target.parentElement.classList.add("focusWithText");
	getLastCoord();
}
function onUserFocus(e) {
	e.target.parentElement.classList.add("focusWithText");
	getUserCoord();
}
...
function onFirstBlur(e) {
	if(e.target.value == "") {
		e.target.parentElement.classList.remove("focusWithText");
	}
	resetFace();
}
function onLastBlur(e) {
	if(e.target.value == "") {
		e.target.parentElement.classList.remove("focusWithText");
	}
	resetFace();
}
function onUserBlur(e) {
	if(e.target.value == "") {
		e.target.parentElement.classList.remove("focusWithText");
	}
	resetFace();
...
if (firstName) {
    firstName.addEventListener('focus', onFirstFocus);
    firstName.addEventListener('blur', onFirstBlur);
    firstName.addEventListener('input', onFirstInput);
}

if (lastName) {
    lastName.addEventListener('focus', onLastFocus);
    lastName.addEventListener('blur', onLastBlur);
    lastName.addEventListener('input', onLastInput);
}

if (username) {
    username.addEventListener('focus', onUserFocus);
    username.addEventListener('blur', onUserBlur);
